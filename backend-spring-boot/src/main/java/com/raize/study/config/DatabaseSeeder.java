...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        if (lessonRepository.count() > 0) {
            logger.info("Database already seeded with lessons. Skipping seeder.");
            return;
        }

        logger.info("🔌 Seeding database from JSON resources...");
        ...
        try {
            ...
            logger.info("✅ Seeded {} exercises successfully.", seedExercises.size());
        } catch (Exception e) {
            logger.error("❌ Error seeding exercises: {}", e.getMessage(), e);
            throw e;
        }
