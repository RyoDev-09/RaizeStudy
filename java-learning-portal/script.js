...
// KHỞI TẠO MONACO SANDBOX CHO MÀN LÝ THUYẾT
function initTheoryMonaco() {
    if (state.theoryEditor) {
        // Đã khởi tạo rồi, chỉ cần layout lại
        setTimeout(() => state.theoryEditor.layout(), 100);
        return;
    }

    const container = document.getElementById('theory-editor-container');
    if (!container) return;

    // Cần đợi Monaco đã được load (từ require(['vs/editor/editor.main']))
    const tryCreate = () => {
        ...
        const lang = isSql ? 'sql' : 'java';
        const starterCode = isSql
            ? `-- Viết câu truy vấn SQL của bạn ở đây\nSELECT 'Hello, SQL!' AS message;`
            : `public class Sandbox {\n    public static void main(String[] args) {\n        // Thực hành theo lý thuyết bên cạnh\n        System.out.println("Hello, Java!");\n    }\n}`;

        state.theoryEditor = monaco.editor.create(container, {
            value: starterCode,
            language: lang,
            theme: 'vs-dark',
            automaticLayout: true,
...
// CHẠY CODE TRONG THEORY SANDBOX
function runTheorySandbox() {
    const terminal = document.getElementById('theory-terminal-output');
    if (!terminal) return;

    if (!state.theoryEditor) {
        terminal.innerHTML = '<span class="terminal-line error-msg">⚠️ Editor chưa sẵn sàng. Vui lòng thử lại sau giây lát.</span>';
        return;
    }

    const code = state.theoryEditor.getValue();
    if (!code.trim()) {
        terminal.innerHTML = '<span class="terminal-line system-msg">ℹ️ Chưa có code để chạy. Hãy viết code Java vào editor!</span>';
        return;
    }

    terminal.innerHTML = '<span class="terminal-line system-msg">⚙️ Đang biên dịch và chạy...</span>';

    setTimeout(() => {
        try {
            const res = runCodeEnv(code);
            if (res.success) {
                const outputLines = (res.output || '[Chương trình kết thúc nhưng không in ra gì]')
                    .split('\n')
                    .map(line => `<span class="terminal-line output">${line}</span>`)
                    .join('');
                terminal.innerHTML = outputLines;
                terminal.innerHTML += '<span class="terminal-line success-msg">✅ Chạy thành công!</span>';
            } else {
                terminal.innerHTML = `<span class="terminal-line error-msg">${res.output}</span>`;
            }
        } catch (e) {
            terminal.innerHTML = `<span class="terminal-line error-msg">❌ Lỗi: ${e.message}</span>`;
        }
    }, 350);
}

// KHỞI TẠO RESIZER CHO THEORY SPLIT LAYOUT
function initTheoryResizer() {
    const resizer = document.getElementById('theory-resizer');
    const layout = document.querySelector('.theory-split-layout');
    ...
    resizer.addEventListener('mousedown', (e) => {
        ...
        const cols = window.getComputedStyle(layout).gridTemplateColumns.split(' ');
        const startLeftWidth = parseFloat(cols[0]);
        ...
        const onMouseMove = (ev) => {
            ...
            layout.style.gridTemplateColumns = `${newLeft}px 5px 1fr`;
            if (state.theoryEditor) state.theoryEditor.layout();
        };
